...
def train_step(images,
    ...
    annotation:
      ...
      (i.e., that should change each time you call the function based
      on criteria not passed to the function as args/kwargs).
    Arguments:
        images: Real image batch (a tuple containing the actual tensor or list).
        gen_model: Generator model.
        gen_optimizer: Generator optimizer.
        dis_model: Discriminator model.
        dis_optimizer: Discriminator optimizer.
    ...
    # Unpack the tuple to get the actual tensor or list
    actual_images = images[0] if isinstance(images, tuple) else images

    # Check if actual_images is a list or a TensorFlow tensor
    if isinstance(actual_images, list):
        # If it's a list, convert it to a TensorFlow tensor
        actual_images = tf.convert_to_tensor(actual_images)
    elif not isinstance(actual_images, tf.Tensor):
        raise TypeError("Input 'images' should be a list or a TensorFlow tensor.")

    # Check the data type of actual_images
    if actual_images.dtype != tf.float32:
        # If the data type is not float32, cast it to float32
        actual_images = tf.cast(actual_images, dtype=tf.float32)
    ...
    # Ensure the batch dimension is added to the actual_images tensor
        actual_images = tf.expand_dims(actual_images, axis=0)

    # Check the number of channels in actual_images
        num_channels = actual_images.shape[-1]
    
    # Convert to RGB if working with grayscale
        if num_channels == 1:
            actual_images = tf.image.grayscale_to_rgb(actual_images)
        elif num_channels != 3:
            raise ValueError("Number of channels in the image should be either 1 or 3.")

        real_output = dis_model(actual_images, training=True)
